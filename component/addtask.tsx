...
export default function AddTask({
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    const url = isEditingTask
      ? `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/taskcard/${id}`
      : `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/taskcard`;
  ...
  return (
    <div className="flex flex-col min-h-screen items-center justify-center bg-zinc-50 font-sans dark:bg-black fixed top-0 left-0 w-full h-full z-50 ">
      <div className=" bg-white rounded-lg shadow-md p-4 ">
        <h2 className="text-gray-600 text-xl font-semibold mb-2 text-center">
          {isEditingTask ? "Edit Task" : "Add New Task"}
        </h2>
        <form
          onSubmit={handleSubmit}
          className="w-full max-w-sm flex flex-col gap-2 justify-center align-center"
        >
          <div>
            <label
          ...
          <div>
            ...
            <textarea
              ...
              name="description"
            />
            <RecordVoice setAudioBlob={setAudioBlob} setAudioURL={setAudioURL} audioURL={audioURL}/>
          </div>
          <div>
            <label
              className="block text-gray-700 text-sm font-bold mb-2"
